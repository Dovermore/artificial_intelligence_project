...
class BoardViewCanvas(tkinter.Canvas):
    ...
    def initialise(self):
        ...
        # add file name entry
        self.file_name_entry = tkinter.Entry(self)
        self.create_window(100, 50, window=self.file_name_entry)

        # exited pieces button
    ...
    def clicked(self, event):
        ...
        if len(click):
            ...
            if theid in self.board.hexid:
                ...
                thebutton.click(self.file_name_entry.get(), self.exited.get())
                return
...
class ExportButton:
    ...
    def click(self, filename, exited):
        tikz = self.render_to_tikz(self.board)
        self.export_json(self.board, filename, exited)
        print(tikz)
    ...
    def export_json(self, board, filename, exited):
        convertion = {'R':'red', 'G':'green', 'B':'blue'}
        dd = {}
        # TODO change to any color
        player = 'red'
        dd['colour'] = player
        for color in convertion.values():
            dd[color] = []
        print(board.pieces)
        for pos in board.pieces:
            piece = board.pieces[pos]
            if piece.col is None:
                continue
            color = convertion[piece.col]
            dd[color].append(list(map(int, pos)))
        dd['completed'] = {}
        for i, value in enumerate(exited.split(',')):
            dd['completed'][list(convertion.values())[i]] = int(value)
